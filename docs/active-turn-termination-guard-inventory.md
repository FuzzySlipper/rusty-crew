...
| Responses repeated identical function calls | Rust `openai-responses` shared no-progress policy | 3 equivalent failed repetitions after the baseline by default; configurable | intent, result, replay state, assistant progress, and success/failure class | successful repetitions continue; confirmed equivalent failure durably pauses as `attention_required` | no-progress state and typed Responses continuation are checkpointed | Landed in task 6368. Explicit retry/cancel resolution is Rust-owned. |
| Malformed Chat Completions tool calls | Rust `chat-completions` shared no-progress policy | 3 equivalent malformed repetitions after the baseline by default; configurable | provider finish reason, malformed summary, durable message state, and partial assistant output | sends model-visible correction, then durably pauses as `attention_required` when unchanged | completed rounds and recovery state survive in the continuation checkpoint | Landed in task 6368; malformed output no longer exhausts into a failed logical turn. |
