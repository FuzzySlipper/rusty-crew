...
| Tool call finish | pi `tool_execution_end` emits `tool_call_finished` with `isError` and the same debug detail reference. | Preserve event shape and debug detail continuity. Tool failures should be delivered to the model for recoverable tool-result rounds where possible. Repeated unrecoverable failures should become visible `provider_status`/`wake_failed` according to the tool failure policy. |
| Tool result format | `BrainToolResult` text/image content is converted to pi tool result content; terminate flag is preserved. | Preserve text/image result support and terminate semantics in the neutral bridge path. |
