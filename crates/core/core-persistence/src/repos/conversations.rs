...
fn insert_or_replace_context_compaction_artifact(
    ...
    conn.execute(
        ...
        ON CONFLICT(artifact_id) DO UPDATE SET
            ...
            logical_turn_id = excluded.logical_turn_id,
            execution_epoch_id = excluded.execution_epoch_id,
            source_projection_fingerprint = excluded.source_projection_fingerprint,
            trigger = excluded.trigger,
            before_tokens = excluded.before_tokens,
            after_tokens = excluded.after_tokens,
            preserved_item_count = excluded.preserved_item_count,
            excised_item_count = excluded.excised_item_count,
            intent_key = excluded.intent_key,
            terminal_status = excluded.terminal_status,
            provider_chain_action = excluded.provider_chain_action,
        ...
        params![
            ...
            artifact.strategy_revision.as_deref(),
            artifact.logical_turn_id.as_deref(),
            artifact.execution_epoch_id.as_deref(),
            artifact.source_projection_fingerprint.as_deref(),
            artifact.trigger.as_deref(),
            artifact.before_tokens.map(|value| value as i64),
            artifact.after_tokens.map(|value| value as i64),
            artifact.preserved_item_count.map(|value| value as i64),
            artifact.excised_item_count.map(|value| value as i64),
            artifact.intent_key.as_deref(),
            artifact.terminal_status.as_deref(),
            artifact.provider_chain_action.as_deref(),
...
fn row_to_context_compaction_artifact(
    ...
    let source_refs_json: String = row.get(16)?;
    let provider_metadata_json: String = row.get(17)?;
    let estimate_before_json: String = row.get(18)?;
    let estimate_after_json: Option<String> = row.get(19)?;
    let enters_future_context: i64 = row.get(21)?;
    let artifact = ContextCompactionArtifact {
        ...
        strategy_revision: row.get::<_, Option<String>>(4)?,
        logical_turn_id: row.get::<_, Option<String>>(5)?,
        execution_epoch_id: row.get::<_, Option<String>>(6)?,
        source_projection_fingerprint: row.get::<_, Option<String>>(7)?,
        trigger: row.get::<_, Option<String>>(8)?,
        before_tokens: row.get::<_, Option<i64>>(9)?.map(|value| value as u64),
        after_tokens: row.get::<_, Option<i64>>(10)?.map(|value| value as u64),
        preserved_item_count: row.get::<_, Option<i64>>(11)?.map(|value| value as u64),
        excised_item_count: row.get::<_, Option<i64>>(12)?.map(|value| value as u64),
        intent_key: row.get::<_, Option<String>>(13)?,
        terminal_status: row.get::<_, Option<String>>(14)?,
        provider_chain_action: row.get::<_, Option<String>>(15)?,
        ...
        estimate_after_json: estimate_after_json
            ...
            .transpose()?,
        summary_text: row.get(20)?,
        enters_future_context: enters_future_context != 0,
        context_policy: row.get(22)?,
        metadata_json: from_json_text(&row.get::<_, String>(23)?).map_err(to_sql_error)?,
