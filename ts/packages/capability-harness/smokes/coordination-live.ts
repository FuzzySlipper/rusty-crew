...
export type CoordinationCertificationRole = "production" | "debug";

export async function runCoordinationLiveCertification(
  deploymentRole: CoordinationCertificationRole,
): Promise<void> {
  const serviceBaseUrl =
    deploymentRole === "debug"
      ...
      : "http://127.0.0.1:9347";
  const runtimeId =
    deploymentRole === "debug" ? "rv-live-codex-5516" : "rusty-crew-live-codex";
  const artifactRoot =
    process.env.RUSTY_CREW_CAPABILITY_ARTIFACT_ROOT ??
    `/tmp/rusty-crew-coordination-${deploymentRole}-${Date.now()}`;
  const timeoutMs = Number(
    process.env.RUSTY_CREW_CAPABILITY_TURN_TIMEOUT_MS ?? 180_000,
  );
  const runId = `coordination-${deploymentRole}-${Date.now()}-${randomUUID().slice(0, 8)}`;
  const fixturePrefix = runId.toLowerCase().replace(/[^a-z0-9-]/gu, "-");
  const directProfileId = `${fixturePrefix}-direct`;
  const directSessionId = `${directProfileId}-session`;
  const directAgentId = `${directProfileId}-agent`;
  const codexProfileId = `${fixturePrefix}-codex`;
  const directProviderAlias =
    deploymentRole === "debug" ? "tester-chat" : "deepseek-flash";
  ...
  interface ExternalAgentSessionCreationResult {
    creation: {
      binding: ExternalBinding;
      session: { agentId: string; sessionId: string; profileId: string };
      nativeThreadId: string;
    };
  }

  interface CoordinationFixture {
    senderBinding: ExternalBinding;
    peerBinding: ExternalBinding;
    preexistingAgentIds: Set<string>;
    preexistingBindingIds: Set<string>;
  }
  ...
  const scenario = validateCapabilityScenario({
    ...
    fixture: {
      kind: "directory",
      sourceRef: `service://${deploymentRole}/coordination`,
    },
    requiredCapabilities: [
      ...
      "ttl_expiry",
      ...(deploymentRole === "debug" ? ["restart_reconciliation"] : []),
    ],
    permittedEffects: [
      `${deploymentRole}_service_turns`,
      ...(deploymentRole === "debug" ? ["debug_service_restart"] : []),
  ...
  try {
    await executeCertification(fixture);
  } finally {
    await cleanupCoordinationFixture(fixture);
  }

  async function executeCertification(
    fixture: CoordinationFixture,
  ): Promise<void> {
    const { senderBinding, peerBinding } = fixture;
    ...
    console.log("[coordination] direct and Codex directory discovery");
    const discovery = await runDirectoryDiscovery(senderBinding);
    ...
    console.log(
      deploymentRole === "debug"
        ? "[coordination] pending round restart and expiry"
        : "[coordination] pending round expiry without service restart",
    );
    ...
    const directRuntime = runtimeEvidence({
      ...
      restart: {
        exercised: deploymentRole === "debug",
        recovered: restart.serviceRecovered,
        evidence:
          deploymentRole === "debug"
            ? `pending round ${restart.round.roundId} survived debug restart and became ${restart.round.status}`
            : `pending round ${restart.round.roundId} became ${restart.round.status} without a production restart`,
      },
    ...
    const codexRuntime = runtimeEvidence({
      ...
      restart: {
        ...
        evidence:
          deploymentRole === "debug"
            ? `controller generation ${restart.controllerGenerationBefore} -> ${restart.controllerGenerationAfter}; round terminal ${restart.round.status}`
            : `controller generation remained ${restart.controllerGenerationBefore}; round terminal ${restart.round.status}`,
      },
      effectiveConfig: {
        runtimeId,
        senderBindingId: senderBinding.bindingId,
        peerBindingId: peerBinding.bindingId,
  ...
  async function assertDeploymentIdentity(): Promise<void> {
    const prefix =
      deploymentRole === "debug"
        ? "/v1/debug/coordination"
        : "/v1/coordination";
    const directory = await getJson<{
      deploymentRole: string;
      agents: Array<{ agentId: string }>;
    }>(`${prefix}/agents`);
    assert.equal(directory.deploymentRole, deploymentRole);
  }

  async function createCoordinationFixture(): Promise<CoordinationFixture> {
    const prefix =
      deploymentRole === "debug"
        ? "/v1/debug/coordination"
        : "/v1/coordination";
    const beforeDirectory = await getJson<{
      agents: Array<{ agentId: string; profileId: string }>;
    }>(`${prefix}/agents`);
    const beforeBindings = await listBindings();
    const preexistingAgentIds = new Set(
      beforeDirectory.agents
        .filter(
          (agent) =>
            !agent.profileId.startsWith(`coordination-${deploymentRole}-`),
        )
        .map((agent) => agent.agentId),
    );
    const preexistingBindingIds = new Set(
      beforeBindings.map((binding) => binding.bindingId),
    );
    let directCreated = false;
    let codexCreated = false;
    let senderBinding: ExternalBinding | undefined;
    let peerBinding: ExternalBinding | undefined;
    try {
      await postAdminControl("/v1/admin/control/profiles", {
        profileId: directProfileId,
        displayName: `${deploymentRole} coordination direct cert`,
        providerAlias: directProviderAlias,
        kind: "full",
        localToolProfileId: "full_agent",
        sessionId: directSessionId,
        agentId: directAgentId,
        reason: `coordination certification ${runId}`,
      });
      directCreated = true;
      await postAdminControl("/v1/admin/control/profiles", {
        profileId: codexProfileId,
        displayName: `${deploymentRole} coordination Codex cert`,
        providerAlias: directProviderAlias,
        kind: "full",
        localToolProfileId: "full_agent",
        sessionId: `${codexProfileId}-profile-session`,
        agentId: `${codexProfileId}-profile-agent`,
        reason: `coordination certification ${runId}`,
      });
      codexCreated = true;
      const sender = await postJson<ExternalAgentSessionCreationResult>(
        "/v1/external-agent-sessions",
        {
          idempotencyKey: `${runId}-codex-sender`,
          runtimeId,
          profileId: codexProfileId,
          cwd: "/home/dev/rusty-crew",
          label: `${deploymentRole} coordination Codex sender`,
        },
      );
      const peer = await postJson<ExternalAgentSessionCreationResult>(
        "/v1/external-agent-sessions",
        {
          idempotencyKey: `${runId}-codex-peer`,
          runtimeId,
          profileId: codexProfileId,
          cwd: "/home/dev/rusty-crew",
          label: `${deploymentRole} coordination Codex peer`,
        },
      );
      senderBinding = sender.creation.binding;
      peerBinding = peer.creation.binding;
      assert.equal(senderBinding.status, "active");
      assert.equal(peerBinding.status, "active");
      assert.equal(typeof senderBinding.nativeThreadId, "string");
      assert.equal(typeof peerBinding.nativeThreadId, "string");
      return {
        senderBinding,
        peerBinding,
        preexistingAgentIds,
        preexistingBindingIds,
      };
    } catch (error) {
      for (const binding of [senderBinding, peerBinding]) {
        if (typeof binding?.nativeThreadId === "string") {
          await deleteNativeThread(binding.nativeThreadId).catch(
            () => undefined,
          );
        }
      }
      if (codexCreated)
        await hardDeleteProfile(codexProfileId).catch(() => undefined);
      if (directCreated)
        await hardDeleteProfile(directProfileId).catch(() => undefined);
      throw error;
    }
  }

  async function cleanupCoordinationFixture(
    fixture: CoordinationFixture,
  ): Promise<void> {
    for (const binding of [fixture.senderBinding, fixture.peerBinding]) {
      if (typeof binding.nativeThreadId === "string") {
        await deleteNativeThread(binding.nativeThreadId);
      }
    }
    await hardDeleteProfile(codexProfileId);
    await hardDeleteProfile(directProfileId);

    const prefix =
      deploymentRole === "debug"
        ? "/v1/debug/coordination"
        : "/v1/coordination";
    const afterDirectory = await getJson<{
      agents: Array<{ agentId: string; profileId: string }>;
    }>(`${prefix}/agents`);
    const afterBindings = await listBindings();
    assert.equal(
      afterDirectory.agents.some(
        (agent) =>
          agent.profileId === directProfileId ||
          agent.profileId === codexProfileId,
      ),
      false,
    );
    assert.equal(
      afterBindings.some(
        (binding) =>
          binding.bindingId === fixture.senderBinding.bindingId ||
          binding.bindingId === fixture.peerBinding.bindingId,
      ),
      false,
    );
    for (const agentId of fixture.preexistingAgentIds) {
      assert(
        afterDirectory.agents.some((agent) => agent.agentId === agentId),
        `pre-existing agent ${agentId} disappeared during certification`,
      );
    }
    for (const bindingId of fixture.preexistingBindingIds) {
      assert(
        afterBindings.some((binding) => binding.bindingId === bindingId),
        `pre-existing binding ${bindingId} disappeared during certification`,
      );
    }
  }
  ...
  async function runDirectoryDiscovery(
    ...
    const directMessageId = `${runId}-direct-directory`;
    const directResponse = await fetch(
      `${serviceBaseUrl}/v1/chat/sessions/${encodeURIComponent(directSessionId)}/messages`,
      {
        ...
        headers: {
          ...
          "idempotency-key": directMessageId,
        },
        body: JSON.stringify({
          actor: { id: "capability-harness", kind: "human" },
          body: "Call list_agents exactly once, then finish the turn without calling another tool.",
          client_message_id: directMessageId,
          reason: "same-service directory certification",
        }),
        signal: AbortSignal.timeout(timeoutMs),
      },
    );
    assert.equal(directResponse.status, 202, await directResponse.text());
    const directEvents = await readChatUntilTerminal(chatCursor);
    assert(
      directEvents.some(
        (event) =>
          event.kind === "tool_call_completed" &&
          event.payload.tool_name === "list_agents" &&
          event.payload.is_error !== true,
      ),
      "direct list_agents tool did not complete",
    );

    const codexCursor = await latestExternalSequence();
    await postBindingMessage(
      binding.bindingId,
      `${runId}-codex-directory`,
      "Call rusty_crew.list_agents exactly once, then finish the turn without calling another tool.",
      60_000,
    );
    const codexTool = await waitForExternalTool({
      afterSequence: codexCursor,
      threadId: requiredString(binding.nativeThreadId),
      tool: "list_agents",
      terminal: true,
    });
    assert.equal(codexTool.completed?.payload.success, true);
    await waitForExternalTurnTerminal(
      codexCursor,
      requiredString(codexTool.started.nativeTurnId),
    );
    return {
      directEvents,
      codexTool,
  ...
  async function runRestartExpiry(binding: ExternalBinding): Promise<{
    ...
    const roundTimeoutMs = deploymentRole === "debug" ? 12_000 : 2_000;
    const targetDelaySeconds = deploymentRole === "debug" ? 30 : 5;
    const body = agentRoundPrompt(
      directAgentId,
      correlationId,
      `Call terminal exactly once to run sleep ${targetDelaySeconds}. After it finishes, call send_agent_message with toAgentId ${binding.agentId}, body LATE_REPLY_${runId}, and correlationId ${correlationId}.`,
      true,
      roundTimeoutMs,
    );
    const delivery = await postBindingMessage(
    ...
    if (deploymentRole === "debug") {
      ...
      assert.equal(after.controllerGeneration, before.controllerGeneration);
    }
  ...
  async function waitForExternalTool(input: {
    ...
    while (Date.now() < deadline) {
      ...
      if (
        started !== undefined &&
        (!input.terminal || completed !== undefined)
      ) {
  ...
  function requireBinding(
    ...
    assert(
      binding !== undefined,
      `external binding ${bindingId} was not found`,
    );
    return binding;
  }
  ...
  async function hardDeleteProfile(profileId: string): Promise<void> {
    const deadline = Date.now() + timeoutMs;
    while (Date.now() < deadline) {
      const data = await postJson<{
        outcome: { status: string; summary?: string; reasonCode?: string };
      }>(`/v1/admin/control/profiles/${encodeURIComponent(profileId)}/delete`, {
        confirmProfileId: profileId,
        reason: `coordination certification cleanup ${runId}`,
      });
      if (data.outcome.status === "completed") return;
      if (
        data.outcome.status !== "failed" ||
        !data.outcome.summary?.includes("blocked by in-flight wake")
      ) {
        assert.fail(JSON.stringify(data.outcome));
      }
      await delay(250);
    }
    throw new Error(`profile ${profileId} remained busy during cleanup`);
  }
  ...
  async function deleteNativeThread(threadId: string): Promise<void> {
    const deadline = Date.now() + timeoutMs;
    const path = `/v1/external-runtimes/${encodeURIComponent(runtimeId)}/threads/${encodeURIComponent(threadId)}/delete`;
    while (Date.now() < deadline) {
      const response = await fetch(`${serviceBaseUrl}${path}`, {
        method: "POST",
        signal: AbortSignal.timeout(30_000),
      ...
      if (response.status === 200 && envelope.ok) return;
      if (
        response.status !== 409 ||
        envelope.error?.reason_code !== "external_thread_active"
      ) {
        throw new Error(
          `native thread cleanup failed ${response.status}: ${JSON.stringify(envelope)}`,
        );
      }
      await delay(250);
    }
    throw new Error(`native thread ${threadId} remained active during cleanup`);
  }
