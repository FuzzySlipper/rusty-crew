...
test("send derives trusted external identity outside model arguments", async () => {
...
test("agent round returns the durable Rust reply", async () => {
  const port = new RecordingPort();
  const result = await resolveCodexCoordinationToolCall({
    params: {
      threadId: "thread-2",
  ...
  assert.deepEqual(port.rounds[0]?.caller, {
    type: "external_agent",
    runtimeId: "codex-local",
    bindingId: "binding-2",
    controllerInstanceId: "controller-2",
...
class RecordingPort implements CodexCoordinationPort {
  ...
  #round: AgentCorrelatedRound | undefined;
  ...
  async beginAgentRound(
    ...
    return { round: this.#round, delivery: deliveryReceiptForRound(command) };
  }
...
function deliveryReceiptForRound(
  command: AgentRoundCommand,
): AgentMessageDeliveryReceipt {
