...
const engineDataDir = mkdtempSync(
  join(tmpdir(), "rusty-crew-production-local-tools-engine-"),
);
const workdir = mkdtempSync(
  join(tmpdir(), "rusty-crew-production-local-tools-workdir-"),
);
writeFileSync(
  join(workdir, "note.txt"),
  "production local tool proof\n",
  "utf8",
);
...
class ToolCallingFakeAgent {
  ...
  async prompt(
    ...
    await this.callTool("read_file", { path: "note.txt" });
    await this.callTool("terminal", {
      command: "printf production-terminal-ok",
      timeoutMs: 5_000,
    });
...
try {
  const sessionId = "production-local-tools-session" as SessionId;
  const agentId = "production-local-tools-agent" as AgentId;
  const profileId = "production-local-tools-profile" as ProfileId;
  const wakeId = "production-local-tools-wake";
  ...
  const brain = await registerBrainImplementationRuntime(
    ...
    createPiAgentBrain({
      createAgent: (options) => new ToolCallingFakeAgent(options, toolOutputs),
      ...
      planActions: ({ wake }): BrainAction[] => [
        {
          ...
          packet: {
            ...
            status: "completed",
            summary: "production local tool wake completed",
          },
        },
      ],
    }),
  ...
  const request = await native.buildBrainWakeRequest({
    ...
    systemPrompt: "Use the selected production local tools.",
    roleAssemblyJson: encoder.encode(
      JSON.stringify({
        instructions: "Read note.txt and run the terminal proof command.",
      }),
    ),
    wakeId,
  });
  ...
  assert.match(toolOutputs.read_file, /production local tool proof/);
  assert.match(toolOutputs.terminal, /production-terminal-ok/);
